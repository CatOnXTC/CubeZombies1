...
void init() {
    ...
    window = SDL_CreateWindow("Between Zombies", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, B, H, SDL_WINDOW_SHOWN);
...
void load(){
    ...
    if(key[0]==NULL)printf("SDL Error: %s\n",SDL_GetError());

    key[1] = loadBild("data/ball.png");
...
while(quit==0){
   while(SDL_PollEvent(&e)!=0){
    ...
    p=anfang;
    while(p!=NULL){
    SDL_RenderCopy(render, bild, NULL, &(p->rect));
    p=p->next;
    }
    ...
    zp=zanfang;
    while(zp!=NULL){
    SDL_RenderCopy(render, bild, NULL, &(zp->zrect));
    zp=zp->znext;
    }
